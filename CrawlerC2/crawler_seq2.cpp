...
int main(void)
{
    ...
    std::chrono:: duration<double> tempoMedioProdutos;
    ...
    std::string html_page = curl_downloadHTML(url);
    std::string novo_html = download_prods_links(html_page)[0];
    std::string html_page2 = curl_downloadHTML(novo_html);
    get_prod_info(html_page2);
    ...
    cout << "Tempo total de Execu;cão do Crawler: " << tempoTotalCrawler1 << '\n';
