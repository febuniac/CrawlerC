...
void download_prods_links(std::string html_pag){
    ...
    for (int i = 0; i < list_link_products.size(); ++i){
            std::cout <<"Link Produto:"<< list_link_products.front() << '\n';
    }
}
std::list< string > list_link_nexts;
std::string download_next_page(std::string html_pag){
    CURL *curl; 
    ...
    for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
        ...
        for (int i = 0; i < list_link_nexts.size(); ++i){
                std::cout <<"Página Next:"<<i<< list_link_nexts.front() << '\n';
        }
    return link_com_site_antes_n;//return link da pagina de next
}
void download_produts_links_LOOP(std::string &url){
    std::string vazio ="";
    while(url != vazio){
...
// void re  gex_parseHTML_prods(std::string html_pag){
//     std::regex linksprod_reg("<a class=\"card-product-url\" href=\"([^\"]+)\"");
//     auto words_begin =
//     std::sregex_iterator(html_pag.begin(), html_pag.end(), linksprod_reg);
//     auto words_end = std::sregex_iterator();
...
int main(void)
{
    ...
    download_produts_links_LOOP(url);
    ...
    //Criando Threads
    std::thread Thread1(download_produts_links_LOOP, std::ref(url));//, std::ref(url),std::ref(),std::ref()
    // std::thread Thread2;
    // std::thread Thread3;
    return 0;
}
